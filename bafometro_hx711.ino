...
// Sensor MQ-3 (Bafômetro)
#define PINO_MQ3 34
const int LIMITE_ALCOOL_RAW = 3000;

// HX711 (Balança)
#define HX711_DOUT 33   // Pino DOUT do HX711
#define HX711_SCK  32   // Pino SCK  do HX711
const float FATOR_CALIBRACAO = 695238;
const float LIMITE_PESO_KG   = 0.5;   // LED acende acima deste peso (kg)

// LED de alerta de peso
#define PINO_LED 2

// NTP (UTC-3, sem horário de verão)
...
// Balança
float  currentPeso   = 0.0;

// Timers
unsigned long previousLcdMillis  = 0;
unsigned long previousLogMillis  = 0;
unsigned long previousPesoMillis = 0;

const long lcdInterval   = 3000;  // Alterna LCD a cada 3 s
const long logInterval   = 10000; // Grava log a cada 10 s
const long pesoInterval  = 500;   // Lê balança a cada 500 ms

bool telaBalanca = false; // Controla qual tela está ativa no LCD
...
void handleRoot() {
  ...
  html += "<hr>\n";
  html += "<h2>Balanca (HX711)</h2>\n";
  html += "<p>Peso: <strong id='pesoVal'>0.00</strong> kg</p>\n";
  html += "<hr>\n";
  ...
  html += "document.getElementById('pesoVal').innerText = data.peso;\n";
...
void handleData() {
  ...
  json += "\"peso\":\""  + String(currentPeso, 2)          + "\",";
...
void setup() {
  Serial.begin(115200);
  delay(200); // Estabilidade no boot
  ...
  if (file.size() == 0) {
    // Cabeçalho atualizado com coluna de peso
    file.println("Data_Hora,Bruto,PPM,Status,Peso_kg");
  }
  file.close();
  delay(100);
  ...
  if (balanca.is_ready()) {
    ...
    balanca.tare(); // Zera a balança no boot
    Serial.println("HX711 pronto e regulado.");
  } else {
    Serial.println("AVISO: HX711 nao encontrado no boot.");
  }
...
void loop() {
  ...
  // --- Leitura da Balança (a cada pesoInterval ms) ---
  if (currentMillis - previousPesoMillis >= pesoInterval) {
    previousPesoMillis = currentMillis;

    if (balanca.is_ready()) {
      currentPeso = balanca.get_units(3); // Média de 3 leituras
      if (currentPeso < 0) currentPeso = 0.0; // Evita valores negativos
    }
    // Se não estiver pronta, mantém o último valor lido (sem travar)

    // Controle do LED pelo peso
    digitalWrite(PINO_LED, (currentPeso > LIMITE_PESO_KG) ? HIGH : LOW);
  }

  // --- Leitura do MQ-3 e alternância do LCD (a cada lcdInterval ms) ---
  if (currentMillis - previousLcdMillis >= lcdInterval) {
    previousLcdMillis = currentMillis;

    // Sempre atualiza leitura do bafômetro
    ...
    if (telaBalanca) {
      // Tela 1: Peso da balança
      lcd.setCursor(0, 0);
      lcd.print("Peso:");
      lcd.setCursor(0, 1);
      lcd.print(currentPeso, 2);
      lcd.print(" kg");
    } else {
      // Tela 2: Bafômetro
      lcd.setCursor(0, 0);
    ...
    telaBalanca = !telaBalanca; // Alterna para próxima tela
  }

  // --- Log em arquivo CSV (a cada logInterval ms) ---
  if (currentMillis - previousLogMillis >= logInterval) {
    ...
    if (file) {
      ...
      file.print(",");
      file.println(currentPeso, 2);
      file.close();
      Serial.println("Log salvo: " + getTimeString() + " | Peso: " + String(currentPeso, 2) + "kg");
    }
  }
}
